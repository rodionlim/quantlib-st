...
if TYPE_CHECKING:
    ...
    from quantlib_st.sysdata.mongodb.mongo_connection import mongoDb  # pragma: no cover
    from quantlib_st.sysdata.parquet.parquet_access import (
        ParquetAccess,
    )  # pragma: no cover
    from quantlib_st.sysdata.mongodb.mongo_IB_client_id import (
        mongoIbBrokerClientIdData,  # pragma: no cover
    )
    from quantlib_st.brokers.ib.ib_connection import connectionIB  # pragma: no cover
...
class dataBlob(object):
    def __init__(
        ...
        """
        Set up a data pipeline with standard attribute names, logging, links to DB etc.

        Class names we know how to handle are:
        'ib*', 'mongo*', 'arctic*', 'csv*', 'parquet*'
        """
    ...
    def _get_new_ib_connection(self):
        # Try this 5 times...
        attempts = 0
        failed_ids = []
        ...
        while True:
            try:
                from quantlib_st.brokers.ib.ib_connection import connectionIB
    ...
    @property
    def parquet_access(self):
        from quantlib_st.sysdata.parquet.parquet_access import ParquetAccess
    ...
    def _get_new_mongo_db(self):
        from quantlib_st.sysdata.mongodb.mongo_connection import mongoDb
    ...
    def config(self):
        ...
        if config is None:
            from quantlib_st.sysdata.production_config import get_production_config
...
def identifying_name(
    ...
    original_source_label = lower_split_up_name.pop(0)  # source eg csv, ib, mongo
...
__all__ = ["dataBlob", "identifying_name", "get_parquet_root_directory"]
