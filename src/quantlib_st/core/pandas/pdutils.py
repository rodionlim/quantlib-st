...
from quantlib_st.core.constants import named_object, arg_not_supplied
...
def get_index_of_columns_in_df_with_at_least_one_value(df: pd.DataFrame) -> List[bool]:
    ...
    return list(~df.isna().all().values)  # type: ignore
...
def pd_readcsv(
    ...
    if input_column_mapping is not arg_not_supplied:
        df = remap_columns_in_pd(df, input_column_mapping)  # type: ignore
...
def from_dict_of_values_to_df(
    data_dict: dict,
    ts_index: pd.Index,
    columns: List[str] | named_object = arg_not_supplied,
) -> pd.DataFrame:
    """
    ...
    if columns is not arg_not_supplied:
        data_dict = {keyname: data_dict[keyname] for keyname in columns}  # type: ignore
...
def make_df_from_list_of_named_tuple(
    ...
    field_name_for_index: str | named_object = arg_not_supplied,
):
    """
...
def from_series_to_matching_df_frame(
    ...
    new_df = new_df.reindex(pd_df_to_match.index, method=method)  # type: ignore
