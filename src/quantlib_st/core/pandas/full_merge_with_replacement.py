...
from quantlib_st.core.pandas.merge_data_keeping_past_data import (
    VERY_BIG_NUMBER,
...
from quantlib_st.core.pandas.pdutils import is_a_series, is_a_dataframe
...
def full_merge_of_existing_data(
    ...
    column_to_check_for_spike: str | named_object = arg_not_supplied,
):
    """
    ...
    if check_for_spike:
        merged_data_with_status = spike_check_merged_data(
            ...
            column_to_check_for_spike=column_to_check_for_spike,  # type: ignore
            max_spike=max_spike,
        )
...
def full_merge_of_data_with_both_old_and_new(
    ...
    if is_a_series(old_data):
        ...
        merged_data = full_merge_of_existing_series(
            old_series=old_data, new_series=new_data, keep_older=keep_older  # type: ignore
        )
    else:
        assert is_a_dataframe(new_data)
        merged_data = full_merge_of_existing_dataframe(
            old_data=old_data, new_data=new_data, keep_older=keep_older  # type: ignore
        )
