...
from quantlib_st.core.objects import (
    resolve_data_method,
    resolve_function,
    hasallattr,
)
from quantlib_st.core.pandas.strategy_functions import replace_all_zeros_with_nan
from quantlib_st.core.text import (
...
# Define a type for trading rule specifications
TradingRuleSpec = Union[
    "TradingRule",
    Tuple[Union[Callable, str], List[str], Dict[str, Any]],
    Dict[str, Any],
    Callable,
    str,
]
...
class TradingRule(object):
        ...
        *dataargs are unnamed data items, and **kwargs are named configuration
        ...
        :type trading_rules:
            ...
            to produce a complete TradingRule:
                ...
                str (with path to function eg "quantlib_st.systems.provide.example.rules.ewmac_forecast_with_defaults")
    ...
    @property
    def data_args(self) -> List[Dict[str, Any]]:
    ...
    def _call_with_data(self, list_of_data_for_call: list) -> pd.Series:
        ...
        if not callable(func):
            raise Exception(
                "Trading rule function is not callable or could not be resolved"
            )
        result = func(*list_of_data_for_call, **(other_args or {}))
...
def _repr_trading_rule(rule: TradingRule):
    data = rule.data or []
    data_args = rule.data_args or []
    other_args = rule.other_args or {}
    other_arg_names = list(other_args.keys())
...
class tradingRuleComponents(object):
    def __init__(
        self,
        rule_function: Union[Callable, str],
        data: list,
        other_args: dict,
        data_args: Optional[list] = None,
    ):
        (
            processed_function,
            processed_data,
        ...
        self._rule_function = processed_function
        self._data = processed_data
        self._other_args = processed_other_args
        self._data_args = processed_data_args
    ...
    def _process_inputs(
        ...
        other_args: dict,
        data_args: Optional[list] = None,
    ) -> Tuple[Callable, List[str], Dict[str, Any], List[Dict[str, Any]]]:
        # turn string into a callable function if required
        resolved_function = cast(Callable, resolve_function(rule_function))
        ...
        return (
            ...
            data,
            other_args,
            cast(List[Dict[str, Any]], data_args),
        )
    ...
    @property
    def data_args(self) -> List[Dict[str, Any]]:
        return self._data_args or []

    @property
    def other_args(self) -> Dict[str, Any]:
        return self._other_args or {}
...
def _get_trading_rule_components_depending_on_rule_input(
    rule: TradingRuleSpec, data: Optional[list], other_args: Optional[dict]
    ...
    if _already_a_trading_rule(rule):
        ...
        rule_components = _create_rule_from_existing_rule(
            cast(TradingRule, rule), data=data, other_args=other_args
        )
        ...
        rule_components = _create_rule_from_passed_elements(
            cast(Union[Callable, str], rule), data=data, other_args=other_args
        )
...
def _create_rule_from_existing_rule(
    rule: TradingRule, data: list, other_args: dict
) -> tradingRuleComponents:
...
def _create_rule_from_tuple(rule: tuple, data: list, other_args: dict):
...
def _create_rule_from_dict(
    rule: dict, data: list, other_args: dict
) -> tradingRuleComponents:
...
def _create_rule_from_passed_elements(
    rule: Union[Callable, str], data: list, other_args: dict
) -> tradingRuleComponents:
    rule_components = tradingRuleComponents(
...
def create_variations_oneparameter(baseRule, list_of_args, argname, nameformat="%s_%s"):
    ...
    >>>
    >>> rule=TradingRule(("quantlib_st.systems.provided.rules.ewmac.ewmac_forecast_with_defaults", [], {}))
