...
class pandlCalculation:
    def __init__(
        ...
        capital: pd.Series | float | int | None = None,
        value_per_point: float = 1.0,
        roundpositions: bool = False,
        delayfill: bool = False,
        passed_diagnostic_df: pd.DataFrame | None = None,
    ):
        self._price = price
    ...
    def as_pd_series(self, percent: bool = False):
        if percent:
    ...
    def _percentage_pandl_given_pandl(self, pandl_in_base: pd.Series):
        ...
        elif isinstance(capital, (float, int)):
    ...
    def pandl_in_points(self) -> pd.Series:
        if self.positions is None:
            raise ValueError("Positions are required to compute P&L")
    ...
    @property
    def positions(self) -> pd.Series | None:
    ...
    def _get_passed_positions(self) -> pd.Series | None:
    ...
    def capital(self) -> pd.Series:
        ...
        if isinstance(capital, (float, int)):
...
def apply_weighting(weight: pd.Series, thing_to_weight: pd.Series | None) -> pd.Series:
    if thing_to_weight is None:
        raise ValueError("Cannot apply weighting to None")
